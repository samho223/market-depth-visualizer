...
// this map is mapping the other product ids existing for toggle
...
function App() {
  ...
  useEffect(() => {
    // window.onresize is handler listen to resize event
    // state variable windowWidth is updated whenever window.innerWidth is changed
    window.onresize = () => {
  ...
  useEffect(() => {
    ...
    // document.hidden return bool, it is true when hidden 
    if (typeof document.hidden !== 'undefined') { // Opera 12.10 and Firefox 18 and later support
      hidden = 'hidden';
      visibilityChange = 'visibilitychange';
      // '@ts-ignore' suppress all errors originate on the following line
    } else { // @ts-ignore
    ...
    const handleVisibilityChange = () => {
      // document['hidden'] return PROPERTY named 'hidden' of document. It is not a map.
    ...
    if (typeof document.addEventListener === 'undefined' || hidden === '') {
      ...
      // visibilitychange is an event that is defined
